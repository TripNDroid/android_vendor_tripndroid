function __print_tripndroid_functions_help() {
cat <<EOF
Additional TripNDroid functions:
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch tripndroid_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/tripndroid-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.tripndroid.device | grep -q "$TRIPNDROID_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $TRIPNDROID_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.tripndroid.device | grep -q "$TRIPNDROID_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.tripndroid.device | grep -q "$TRIPNDROID_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $TRIPNDROID_FIXUP_COMMON_OUT ]; then
